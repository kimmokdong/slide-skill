...
```
[모듈 -1] 환경 점검 (Pre-flight) → [모듈 0] 사전 컨텍스트 (Intake) → [모듈 1] 레퍼런스 파싱 및 리서치 보강
...
## 모듈 -1: 환경 세팅 및 의존성 점검 (Pre-flight Check)

스킬을 처음 실행하거나 작업을 시작하기 전에, 시스템에 필수 라이브러리가 설치되어 있는지 확인해야 합니다.
파이썬 스크립트 실행 중 `ModuleNotFoundError` 등이 발생할 것으로 예상되거나 처음 환경인 경우, **터미널 실행 도구(`run_command`)를 이용해 아래 필수 의존성을 직접 설치**해 주거나 사용자에게 안내하세요.
```bash
pip install python-pptx playwright
playwright install chromium
```
환경 점검 및 설치가 완료되면 [모듈 0]으로 진입합니다.
...
**[⚠️강제 규칙: 사전 컨텍스트 역질문]**
사용자가 처음 명령을 내릴 때 아래 7가지 필수 변수 중 하나라도 누락되어 있다면, **임의로 추측하거나 생략하고 넘어가서는 절대 안 됩니다.** 반드시 제공된 `ask_question` 도구를 사용하여 다중 선택형(객관식)으로 역질문하세요.
- 각 질문에는 선택 가능한 보기(Options)를 구체적으로 제시해야 합니다.
- 원문 텍스트 맥락을 분석해 가장 적절해 보이는 보기가 있다면, 보기 텍스트 맨 앞에 `(추천)`을 붙여 첫 번째 옵션으로 제시하세요.
- 사용자가 주관식으로 입력할 수 있는 '기타(Other)' 옵션은 시스템 UI에서 기본 제공되므로 별도로 보기에 추가하지 마세요.
...
수집이 완료되면 `slide_context.yaml`에 저장합니다.
